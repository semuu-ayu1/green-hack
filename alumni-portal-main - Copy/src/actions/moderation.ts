...
export async function issueStrike(targetUserId: string, reason?: string) {
  try {
    ...
    await prisma.notification.create({
      data: {
        userId: targetUserId,
        message: `GbpietNetwork.in Security: You have received a community strike (${uniqueStrikeCount}/3). Subsequent violations will lead to an automated platform ban.`,
        type: 'STRIKE'
      }
    });
    ...
    if (uniqueStrikeCount >= 3) {
      ...
      await prisma.notification.create({
        data: {
          userId: targetUserId,
          message: 'CRITICAL: Your GbpietNetwork.in account has been automatically restricted following 3 cumulative community strikes.',
          type: 'ADMIN'
        }
      });
