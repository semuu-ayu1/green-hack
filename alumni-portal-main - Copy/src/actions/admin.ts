...
export async function resolveDuplicate(userIdToDelete: string) {
  try {
    ...
    revalidatePath('/admin');
    return { success: true, message: 'Duplicate profile purged from GbpietNetwork.in database.' };
  } catch (err) {
  ...
  try {
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `GbpietNetwork.in Admin Action: Your post content has been refined for institutional clarity. You have the right to appeal.`,
        type: 'EDIT',
        targetId: postId
      }
    });
