...
export default function DuplicateReviewList({ duplicates }: { duplicates: any[] }) {
  ...
  async function handlePurge(userId: string) {
    if (!confirm('Are you sure you want to purge this specific profile from GbpietNetwork.in?')) return;
    setLoading(userId);
  ...
  if (duplicates.length === 0) return (
    <div style={{ padding: '2rem', textAlign: 'center', opacity: 0.6, fontSize: '0.9rem' }}>
      🎉 Database Integrity Check: No obvious duplicate names found in GbpietNetwork.in.
    </div>
  );
